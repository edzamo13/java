package ec.workshop.java8.concurrency.waitnotify;
...
public class Drop {
    ...
    // Verdadero si el consumidor debe esperar a que el productor env�e un mensaje
    // Falso si el productor debe esperar a que el consumidor tome el mensaje
    private boolean empty = true;

    public synchronized String take() {
    	
    	// Esperamos a que est� disponible
        while (empty) {
            try {
                wait();
    ...
    public synchronized void put(String message) {
        // Esperamos a que el mensaje haya sido le�do
        while (!empty) {
            try { 
                wait();
            } catch (InterruptedException e) {}
